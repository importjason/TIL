...
Returning ```0``` seems unnecessary, but the computer can judge if the process ended successfully by it. But actually, ```return 0``` for ```int main()``` can be omitted from C99 because the compiler adds it automatically.
